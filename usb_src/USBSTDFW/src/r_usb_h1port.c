...
#endif  /* __RX */
{
    status_t    intsts0, intsts1, brdysts, nrdysts, bempsts;
    status_t    intenb0, intenb1, brdyenb, nrdyenb, bempenb;
    intbit_t    ists_u;
    intbit2_t   ists1_w;
#ifdef  __RX
...
{
#ifdef USB_HOST_BC_ENABLE
    usb_hstd_bc_func[g_usb_hstd_bc[USB_PORT0].state][USB_BC_EVENT_DT]();
...
USB_STATIC void usb_hstd_detach_process(void)
{
    ...
    for( g_usb_HcdDevAddr = USB_DEVICE_0; g_usb_HcdDevAddr <= USB_MAXDEVADDR; g_usb_HcdDevAddr++ )
    {
        if( usb_hstd_check_device_address() != USB_NOCONNECT )
        {
            ...
            usb_hstd_set_devaddx_register(USB_NOCONNECT, USB_PORT0);
        }
    }
...
#ifdef USB_HOST_BC_ENABLE
    usb_hstd_bc_func[g_usb_hstd_bc[USB_PORT0].state][USB_BC_EVENT_AT]();
#endif /* USB_HOST_BC_ENABLE */
...
USB_STATIC void usb_hstd_bchg_process(uint16_t buf)
{
    ...
    if ((g_usb_HcdRemotePort[USB_PORT0] == USB_YES) && ((uint16_t)(buf & USB_RESUME) == USB_RESUME))
    {
        /* Change device state to resume */
        g_usb_HcdRemotePort[USB_PORT0] = USB_NO;
...
void usb_hstd_bus_reset(void)
{
   ...
   //asm ("Reset");
    ...
    extern  uint32_t        Sys_DisplayFlags;
    //ZeraEfeito();
    //Sys_DisplayFlags = 0;
    //Sys_DisplayFlags |= NeedToChangeActualMessage;
    //Sys_MessagePointer = 0;12.11
    usb_cpu_delay_xms((uint16_t)100);  //1200
    USB_MDF_PAT(DVSTCTR0, USB_USBRST, (USB_USBRST | USB_UACT));
    ZeraEfeito();
    usb_cpu_delay_xms((uint16_t)50);//1000
    usb_hstd_set_uact_p0();//tem q dar um tempinho antes dessa fcn aqui
    /* Wait 10ms or more (USB reset recovery) */
    //ZeraEfeito();
    usb_cpu_delay_xms((uint16_t)50);//10
    USB_WR(DCPMAXP,(uint16_t)(USB_DEFPACKET + USB_DEVICE_0));
    //ZeraEfeito();
    usb_cpu_delay_xms((uint16_t)40);//250
...
#ifdef USB_HOST_BC_ENABLE
        usb_hstd_bc_func[g_usb_hstd_bc[USB_PORT0].state][USB_BC_EVENT_VB]();
#endif
...
void usb_hstd_suspend_process(void)
{
    ...
    if( g_usb_HcdRemotePort[USB_PORT0] == USB_YES )
    {
