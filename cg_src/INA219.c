...
{
	SoftwareI2C_beginTransmission(ina219_i2caddr);
	SoftwareI2C_sendByteAck(reg);                        // Register
	SoftwareI2C_sendByteAck(value >> 8);                 // Upper 8-bits
	SoftwareI2C_sendByteAck(value & 0xFF);               // Lower 8-bits
	SoftwareI2C_endTransmission();
}
...
void INA219_ReadRegister(uint8_t reg, uint16_t *value)
{
	...
	SoftwareI2C_sendByteAck(reg);                        // Register
	SoftwareI2C_endTransmission();
	__delay_ms(1); // Max 12-bit conversion time is 586us per sample
	SoftwareI2C_requestFrom(ina219_i2caddr, (uint8_t)2);
	*value = ((SoftwareI2C_read() << 8) | SoftwareI2C_read());
}
...
int16_t INA219_getPower_raw(void)
{
  ...
  INA219_ReadRegister(INA219_REG_POWER, &value);
...
/**************************************************************************/
/*!
    @brief  Gets the power value in mW, taking into account the config settings and current LSB
